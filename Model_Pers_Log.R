...
data <- list("Y","nplot","nyear","n.visits","nspec","Logging","Unlogged","Logged")
...
parameters <- c("pspp","beta0","beta1","beta.Logging","psi.Unlogged","psi.Logged")
...
for(i in 1:nspec){
  ...
  for(j in 1:nplot){  # Loop for year 1 - uses z0 to calculate persistence offset 
    ...
    logit(psi[j,i,1]) <- beta0[i] + beta1[i]*z0[j,i] + beta.Logging[i]*Logging[j]
    ...
    for(t in 2:nyear){  # Loop through remaining years 2 and up - uses Z[,,t-1] to calculate persistence offset
      ...
      logit(psi[j,i,t]) <- beta0[i] + beta1[i]*Z[j,i,(t-1)] + beta.Logging[i]*Logging[j]
      ###Model for detection probability ###
...
#save results
library(R.utils)
saveObject(bugout,"Mod_Pers_Log")
