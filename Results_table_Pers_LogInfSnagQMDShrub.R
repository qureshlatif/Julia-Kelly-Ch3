...
library(R.utils)
mod <- loadObject("Mod_Pers_LogInfSnagQMDShrub")

#Functions
logit <- function(x) log(x/(1-x))
expit <- function(x) exp(x)/(1+exp(x))

##_______ Tabulate species slope parameter estimates and 95% credible intervals ____________________________##
rows<-spp
cols<-c("p","p.lo","p.hi","beta0","beta0.lo","beta0.hi","beta1","beta1.lo",
        "beta1.hi","beta.Logging","beta.Logging.lo","beta.Logging.hi",
        "beta.Infest","beta.Infest.lo","beta.Infest.hi","beta.shrubs","beta.shrubs.lo","beta.shrubs.hi",
        "beta.QMD","beta.QMD.lo","beta.QMD.hi","beta.snag","beta.snag.lo","beta.snag.hi")
...
out[,"beta.shrubs"] <- apply(mod$BUGSoutput$sims.list$beta.shrubs,2,median)  
out[,"beta.shrubs.lo"] <- apply(mod$BUGSoutput$sims.list$beta.shrubs,2,function(x) quantile(x,prob=0.05,type=8))  
out[,"beta.shrubs.hi"] <- apply(mod$BUGSoutput$sims.list$beta.shrubs,2,function(x) quantile(x,prob=0.95,type=8))  
...
write.csv(out,"Beta_Ests_Pers_LogInfSnagQMDShrub.csv",row.names=T)
...
out <- out[which(out[,"beta.Logging.lo"]>0|out[,"beta.Logging.hi"]<0|out[,"beta.Infest.lo"]>0|out[,"beta.Infest.hi"]<0|
                   out[,"beta.shrubs.lo"]>0|out[,"beta.shrubs.hi"]<0|out[,"beta.QMD.lo"]>0|out[,"beta.QMD.hi"]<0|
                   out[,"beta.snag.lo"]>0|out[,"beta.snag.hi"]<0),]
out.betas.supported <- out
write.csv(out,"Beta_supported_Pers_LogInfSnagQMDShrub.csv",row.names=T) #Not needed for now.
rm(out)
...
# Levels for finite-sample estimates:
  ...
  # Shrubs
    # mean x (Low = 8.04, Low = 17.7, Mod/High = 29.6)
    # range z (Low < -0.5, Mod = -0.5 - 0.5, High > 0.5)

# Shrubs #
  # Variables for script generating predicted probabilities #
X.data <- Plot.data$shrubs_per
X <- seq(min(X.data),max(X.data),length.out=20)
ShrubX.plot <- X # Store values along gradient for plotting predicted occupancy probabilities
Z <- (X - mean(X.data))/sd(X.data)
X.mult <- 1 # Multiplier for column names in table of predicted values
B <- "beta.shrubs" # Name of Beta coefficient in BUGS model output
  #_________________________________________________________#

rows<-spp
cols<-c("psiEst.ShrbLo.md","psiEst.ShrbLo.lo","psiEst.ShrbLo.hi",
        "psiEst.ShrbMd.md","psiEst.ShrbMd.lo","psiEst.ShrbMd.hi",
        "psiEst.ShrbHi.md","psiEst.ShrbHi.lo","psiEst.ShrbHi.hi")
cols <- c(cols,paste("psiPred.Shrb",round(X*X.mult),".md",sep=""),
          paste("psiPred.Shrb",round(X*X.mult),".lo",sep=""),
          paste("psiPred.Shrb",round(X*X.mult),".hi",sep=""))
psi.shrb<-matrix(NA,nrow=length(rows),ncol=length(cols))
dimnames(psi.shrb)<-list(rows,cols)

psi.shrb[,"psiEst.ShrbLo.md"] <- apply(mod$BUGSoutput$sims.list$psi.shrblo,2,median)
psi.shrb[,"psiEst.ShrbLo.lo"] <- apply(mod$BUGSoutput$sims.list$psi.shrblo,2,function(x) quantile(x,prob=0.05,type=8))
psi.shrb[,"psiEst.ShrbLo.hi"] <- apply(mod$BUGSoutput$sims.list$psi.shrblo,2,function(x) quantile(x,prob=0.95,type=8))

psi.shrb[,"psiEst.ShrbMd.md"] <- apply(mod$BUGSoutput$sims.list$psi.shrbmd,2,median)
psi.shrb[,"psiEst.ShrbMd.lo"] <- apply(mod$BUGSoutput$sims.list$psi.shrbmd,2,function(x) quantile(x,prob=0.05,type=8))
psi.shrb[,"psiEst.ShrbMd.hi"] <- apply(mod$BUGSoutput$sims.list$psi.shrbmd,2,function(x) quantile(x,prob=0.95,type=8))

psi.shrb[,"psiEst.ShrbHi.md"] <- apply(mod$BUGSoutput$sims.list$psi.shrbhi,2,median)
psi.shrb[,"psiEst.ShrbHi.lo"] <- apply(mod$BUGSoutput$sims.list$psi.shrbhi,2,function(x) quantile(x,prob=0.05,type=8))
psi.shrb[,"psiEst.ShrbHi.hi"] <- apply(mod$BUGSoutput$sims.list$psi.shrbhi,2,function(x) quantile(x,prob=0.95,type=8))
...
for(i in 1:length(X)) {
  ...
  psi.shrb[,paste("psiPred.Shrb",round(X[i]*X.mult),".md",sep="")] <- apply(psi,2,median)
  psi.shrb[,paste("psiPred.Shrb",round(X[i]*X.mult),".lo",sep="")] <-
    apply(psi,2,function(x) quantile(x,prob=0.05,type=8))
  psi.shrb[,paste("psiPred.Shrb",round(X[i]*X.mult),".hi",sep="")] <-
...
out <- cbind(out,p=paste(round(out.betas[,"p"],digits=2)," (",round(out.betas[,"p.lo"],digits=2),",",
                         round(out.betas[,"p.hi"],digits=2),")",sep=""))

write.csv(out,"Spp_detections.csv",row.names=F)

##______ Tabulate covariate summaries and correlation with logging________##
dat <- data.frame(cbind(Logging=rep(Plot.data$Logging,2),EarlInf=c(Plot.data$EarlInf_2014,Plot.data$EarlInf_2015),
                        snag=rep(Plot.data$snag,2),QMD=rep(Plot.data$QMD,2),shrub=rep(Plot.data$shrubs_per)))
dat$Trt <- rep(Plot.data$treat_cont,2)
vars <- names(dat)[1:5]
cols <- c("Logged","Unlogged","Correlation")
out <- matrix(NA,nrow=length(vars),ncol=length(cols),dimnames=list(vars,cols))

for(i in 1:length(vars)) {
  out[i,"Logged"] <- paste(round(mean(dat[which(dat$Trt=="t"),i]),digits=2)," (",
                           round(sd(dat[which(dat$Trt=="t"),i]),digits=2),")",sep="")
  out[i,"Unlogged"] <- paste(round(mean(dat[which(dat$Trt=="c"),i]),digits=2)," (",
                           round(sd(dat[which(dat$Trt=="c"),i]),digits=2),")",sep="")
  out[i,"Correlation"] <- round(cor(dat[,i],dat[,1]),digits=3)  
}
