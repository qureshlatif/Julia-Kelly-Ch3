library(R2jags)
library(abind) # Don't think this is used, but left here just in case.
setwd("F:/research stuff/FS_PostDoc/outside_consult/LoggingData_Julia")  # Set workspace to location with 'Data_compiled' workspace.
...
nyear <- length(year)
nspec <- length(spp)
Y <- Y.arry  # Renamed to accommodate model code copied from previous project.

Z.init <- apply(Y,c(1,2,3),function(x) sum(x>=1)) # Initial values for occupancy state parameter (only one really needed).
...
data <- list("Y","nplot","nyear","n.visits","nspec","Logging","snag","QMD","Infest","Unlogged","Logged",
...
parameters <- c("pspp","beta0","beta1","beta.Logging","beta.Infest","beta.snag","beta.QMD",
...
for(i in 1:nspec){
  ...
  for(j in 1:nplot){  # Loop for year 1 - uses z0 to calculate persistence offset 
    ...
    logit(psi[j,i,1]) <- beta0[i] + beta1[i]*z0[j,i] + beta.Logging[i]*Logging[j] + beta.Infest[i]*Infest[j,1] + 
      beta.snag[i]*snag[j] + beta.QMD[i]*QMD[j]
    ...
    for(t in 2:nyear){  # Loop through remaining years 2 and up - uses Z[,,t-1] to calculate persistence offset
      ...
      logit(psi[j,i,t]) <- beta0[i] + beta1[i]*Z[j,i,(t-1)] + beta.Logging[i]*Logging[j] + beta.Infest[i]*Infest[j,t] +
        beta.snag[i]*snag[j] + beta.QMD[i]*QMD[j]
      ###Model for detection probability ###
